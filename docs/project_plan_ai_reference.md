...
**一句话概括：** 基于**知识图谱点亮**的 SQL 自适应刷题平台——按分类逐叶出题（每叶 5 题），连续答对 3 题点亮知识点，29 叶 + 7 枝 + 根节点逐级点亮整棵图谱，一轮至多 145 题闭环刷题，实现**看得见的自适应成长**。

**使用场景：** Web 端（Python Flask + SQLite，自包含 HTML 前端，零构建步骤）

**当前题库规模：** 515 题（58 道选择题 + 457 道填空题），全部含标准答案与预期输出
...
### 2.1 实际答题流程（已实现，2026-08-05 图谱点亮版）

```
登录 → 自适应图谱界面（点亮小灯 + 开始适应）
         ↓
    Journey 主循环：枝不放回 → 叶不放回 → 每叶连续 5 题（3 选择 + 2 填空，选择先行）
         ↓
    用户作答
         ├── ✅ 连续答对 3 道（相邻间隔 ≤2min）→ 点亮该叶节点 → 跳过剩余题
         ├── ✅ 该叶 5 题全部答对 → 点亮该叶节点（不计间隔）
         └── ⭐ 累计答对 10 道该叶类型题（跨自适应/自主/真题）→ 点亮该叶节点
         ↓
    枝节点 = 点亮叶数/叶总数（x/y），全亮点亮；根节点 = 点亮枝数/7（x/7），全亮点亮
         ↓
    一轮（至多 145 题）完成 → 保留点亮状态 → 回到图谱界面重新开始新一轮
```
...
> ⚠️ **2026-08-05 起已废弃**：三阶段（cold/exploration/mastery）与 BKT/Thompson 选题已被「图谱点亮引擎」完全替换（见 3.4）。本节保留作历史设计参考。
...
### 2.3 用户完整旅程（2026-08-05 图谱点亮版）

```
访问门户首页（index_glass.html）→ 登录/注册（login_glass.html）→ 自适应进阶（Journey）
  │                                                     ↓
  │             双循环出题：枝不放回 → 叶不放回 → 每叶 5 题（3 选择 + 2 填空）
  │                                                     ↓
  │                         作答 → 点亮判定（规则1/2/3）→ 出下一题
  │                                                     ↓
  │                       图谱点亮：叶 x/10 → 枝 x/y → 根 x/7
  │                                                     ↓
  │                      一轮（至多 145 题）完成 → 保留点亮 → 新一轮
  │
  └── 也支持手动选题模式（自主练习 / 真题测试，按分类/难度筛选）
```
...
### 3.4 自适应引擎核心算法（已实现，2026-08-05 图谱点亮版）

> 2026-08-05 重写：原「BKT + Thompson 采样 + 三阶段递进 + 题型解锁状态机」引擎已完全替换为「双循环出题 + 图谱点亮」引擎（`backend/engine.py`）。以下为当前实现。

#### 出题流程（双循环）
- 枝节点不放回（固定图谱顺序）→ 叶节点不放回 → 每叶连续 5 题（`[选择×3, 填空×2]` 选择先行）
- 题池不足 5 道时**循环复用**（同题可重复出，user_progress 按记录条数计数）；填空池为空 → 填空槽回退选择题池
- 一轮 = 所有未点亮且有题叶节点刷完（29 叶 × 5 = 至多 145 题）；完成后保留点亮状态，回到图谱界面重新开始新一轮（已点亮叶不进新一轮）

#### 点亮规则（叶节点，任一满足即点亮）
- **规则1**：本轮该叶出的题中连续答对 3 道（相邻两题提交间隔 ≤2min）→ 点亮并跳过剩余题
- **规则2**：本轮该叶全部题均答对（不计间隔）→ 点亮
- **规则3**：累计答对 10 道该叶类型题（自适应+自主练习+真题测试跨池，按 user_progress 派生）→ 点亮；每答对 2 道点亮 20%，节点内显示 `x/10`

#### 点亮规则（枝/根节点）
- **枝节点**：圆圈内显示 `x/y`（点亮叶数/叶总数，来自图谱 top→leaf 边）；全部叶点亮 → 枝点亮
- **根节点**：显示 `x/7`（点亮枝数/枝总数）；全部枝点亮 → 根点亮

#### 数据与接口
- 新表 `user_lights`（规则1/2 事件落库；规则3 由 user_progress 幂等派生）；`journey_state.round_state`（JSON，本轮计划+指针+段内答题记录）
- `journey_next()` 响应含 `lights`（37 节点全量）、`round`（轮进度）、`round_complete`、`lit_now`；`unlocked_nodes` 已移除
...
| 类型 | 数量 | 说明 |
|---|---|---|
| **选择题（MCQ）** | 58 道 | 基础选择 29 + 进阶选择 29（每叶节点 1+1），含 4 选项 + 解析 |
| **填空题** | 457 道 | 含标准答案 + 预期输出（SQLite 执行验证），已翻译中文标题 |
| **合计** | **515 道** | |

> **注意：** 设计文档中规划的 640 题（力扣 323 + 牛客 277 + Kaggle 40）未全部入库。Kaggle 题库（`Kaggle题库/` 目录）和牛客题库（`网络爬虫/` 目录）的原始文件存在，但只有部分导入。当前实际可用 515 题（练习池）+ 50 题（真题池）。
...
| `/admin` | 平台数据总览（统计 + 用户明细含密码管理；需先经统一入口登录） |
| `/admin-gate` | 旧入口（2026-09-03 起 302 → `/admin-auth`，页面文件已删除） |
| `/admin-auth` | 管理后台统一入口：登录 / 内推码注册（无个人密钥，注册成功直接进入后台） |
| `/admin-panel` | 管理后台主面板：用户动态 + 系统日志（全员）+ 管理员动态（主管理员）+ 我的同事/内推码/密码管理 |
...
| `/api/journey/start` | POST | 开始自适应旅程（自动建轮/续接/开新轮） |
| `/api/journey/next` | POST | **自适应核心接口**：提交当前答案 → 点亮判定（规则1/2/3）→ 返回下一题 |
| `/api/journey/status` | POST | 当前旅程状态（state、图谱、掌握度、**lights 全量点亮状态**、round 轮进度） |
| `/api/journey/toggle_deep` | POST | 开关深度模式（选择→填空验证） |

> 响应新增字段（2026-08-05）：`lights`（37 节点全量 `{lit, correct | x, y}`）、`round`（`{status, branch, leaf, pos, leaf_total, answered_total, lit_round}`）、`round_complete`、`lit_now`；`unlocked_nodes` 已移除。
...
| `/api/admin/colleagues` | GET | 我的同事（用户名 + 最后上线时间 + is_primary + rollback_count，响应含 me 资料；身份由 Bearer 会话 token 解析；密钥列已随密钥体系移除） |
| `/api/admin/login` | POST | 管理后台登录：账号密码（密钥已移除）；成功签发 per-admin 会话 token |
| `/api/admin/auth-register` | POST | 管理员内推码注册（403 错码 / 409 重名 / 400 弱密码 / 400 非法用户名）；成功签发 per-admin 会话 token |
| `/api/admin/auth-login` | POST | 管理员账号登录（更新最后上线时间）；签发 per-admin 会话 token |
| `/api/admin/user-activity` | GET | 用户动态（实时，倒序 limit≤300）：注册/登录/登出/答题/摸底；所有管理员可见 |
| `/api/admin/system-logs` | GET | 系统日志（实时，倒序；?level=info\|warn\|error）；所有管理员可见 |
| `/api/admin/admin-activity` | GET | 管理员动态（实时，倒序）：登录/注册/改密/删除/内推码等；**仅主管理员**（403 拦截） |
...
│   ├── engine.py                  ← 自适应引擎核心（journey_next 图谱点亮版）
│   ├── db.py / repositories.py / auth.py / seeding.py  ← 分层：连接/schema / 查询层 / 认证 / 播种
...
| 功能 | 状态 | 位置 |
|---|---|---|
| Flask + SQLite 后端 | ✅ 完成 | `backend/app.py` + `db.py`（分层：repositories/auth/engine/seeding） |
| 知识图谱（37 节点三层结构） | ✅ 完成 | `seeding.py` → `seed_knowledge_graph()` |
| **图谱点亮引擎（双循环出题 + 叶/枝/根三级点亮）** | ✅ 完成（2026-08-05 替换） | `engine.py` → `journey_next()` / `compute_lights()` |
| 点亮状态持久化（user_lights + round_state） | ✅ 完成 | `db.py` + `engine.py`（规则1/2 落库，规则3 派生） |
| 图谱点亮可视化（x/10、x/y、x/7 进度） | ✅ 完成 | `frontend/knowledge_map.html` + `index.html` |
...
| 管理后台登录门（双重验证） | ✅ 完成（2026-09-03 起移除） | 原 `frontend/admin_gate.html` 已删除；`/admin-gate` 302 → `/admin-auth`，改为账号密码登录 |
| **管理后台融合 + 实时监控** | ✅ 完成（2026-09-03） | 「管理后台 + 加入我们」融合为单一入口与主面板；移除个人密钥体系；`activity_log` 用户动态全员可见、`system_logs` 全员可见、管理员动态（kind='admin'）仅主管理员 Yuhuijuan 可见；前端 6s 轮询（`admin_panel.html`），平台数据总览保留于 `/admin` |
...
| **全站 6 语言（先导页主导）** | ✅ 完成 | 语言下拉仅保留在先导页 `index_glass.html`（简体中文/繁體中文/English/日本語/हिन्दी/Português），其余 6 页读取 `localStorage.lang` 跟随；login_glass/about/admin_auth/index 补齐 zh_tw/ja/hi/pt 词条（229 键）；admin_gate/admin_panel 补 `langKey()` 定义（此前引用未定义会报错）；t() 统一回退链 `e[langKey()] || e.en || e.zh`；index.html 分类标签繁体版 `KG_LABELS_TW`（日/印/葡用英文标签）；了解我们动态内容 UPDATES_EXTRA/VISION_EXTRA 6 语言 |
| 知识图谱可视化 | ✅ 完成 | `frontend/knowledge_map.html`（ECharts） |
| **导航布局稳定性** | ✅ 完成 | `frontend/index.html`：修复 header 多余 `</div>` 破坏的 main-wrap 嵌套 + 品牌行 `white-space:nowrap`，错题集/分类筛选等视图切换时导航栏不再上下位移（无头浏览器实测 4 状态恒定） |
...
| **单元测试 / 集成测试** | ✅ 完成 | `backend/tests/`（pytest 149 用例，临时 SQLite 隔离；含判题缓存/超时/差异提示、注册收敛、备份与 schema 版本、动态/日志分页用例，全绿） |
...
| M4 题库建设 | ✅ 已完成 | 515 题（58 MCQs + 457 fill-in）+ 50 真题，含标准答案 + 预期输出 |
...
- **要改自适应逻辑：** 改 `backend/engine.py` → `journey_next()` 函数（2026-08 图谱点亮版，是整个项目的核心）
